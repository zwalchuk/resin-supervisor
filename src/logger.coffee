...
class NativeLoggerBackend
	...
	doPublish: =>
		return if !@apiBinder.readyForLogs() or @offlineMode or !@publishEnabled or @publishQueue.length is 0
		# Silently ignore errors sending logs (just like with pubnub)
		@apiBinder.logBatch(_.slice(@publishQueue, 0, @maxLogLinesPerBatch))
		.catch(->)
		@publishQueue = _.slice(@publishQueue, @maxLogLinesPerBatch)

	logDependent: (msg, { uuid }) =>
		if msg.message?.length > @maxLineLength
			msg = _.clone(msg)
			msg.message = _.truncate(msg.message, { length: @maxLineLength, omission: '[...]' })
